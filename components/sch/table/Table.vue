<script setup>
const { programs, programForm } = useNewProgramsStore().storeRefs()
...
<template>
  <div class="relative overflow-y-auto">
    ...
    <table class="relative min-w-full table-fixed divide-y divide-gray-200 overflow-y-hidden">
      <tbody class="divide-y divide-gray-200 bg-white dark:bg-zinc-800">
        ...
        <SchTableProgramCard v-for="program in programs" :program />
        <div class="absolute bottom-0 left-16 right-0 top-0 border-none bg-zinc-800/40" v-if="programForm.shedules.length"></div>
        <SchTableProgramCard :program="programForm" />
      </tbody>
    </table>
  </div>
</template>
